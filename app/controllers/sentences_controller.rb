  ...
  def index
    ...
    @query = params[:query]
      ...
      # Try putting % % around :query -> https://sqlbolt.com/lesson/select_queries_with_constraints_pt_2
  		@raw_sentences = Sentence.joins(:bodies).where(sql_query, query: "%#{params[:query]}%")
      @query_qty = @raw_sentences.each_with_object(Hash.new(0)) { |sentence, counts| counts[sentence] += 1 }
